...
```{r CD4abs, fig.cap=sprintf("%s colored according to before/after ART start date.", varname), fig.width=10, fig.height=10}
...
```{r CD4absdaydistr, fig.cap=sprintf("Distribution of %s and time points before and after transformation, colored according to subjid. %s is %s and day is %s.", varname, varname, tr(CVsplines[[varid]]$lambda$y), tr(CVsplines[[varid]]$lambda$x)), warning=FALSE}
CVsplines[[varid]]$pl$distr  
```

```{r CD4absspar, fig.cap="Sum of squares versus spar for CD4 count."}
CVsplines[[varid]]$pl$spar
```

```{r CD4absbeforeART, fig.width=10, fig.height=10, fig.cap="CD4 count before start of ART. The smoothing spline is shown in red.", warning=FALSE}
...
```{r NCD4abs, fig.cap="Number of patients with CD4 count observation per time point.", fig.width=14}
...
### Cluster CD4 count profiles based on Euclidean distance


Based on the cubic spline predicted CD4 count at evenly spread (on transformed scale) time points the Euclidean distance between patients CD4 count curves can be calculated. As no extrapolation is done, no value will be availbale for time points outside the observed range. Hence, the number of patients that can be compared depend on the studied time interval.
...
```{r EuclideanclustersCD4abs, warning=FALSE}
timewindows <- as.list(as.data.frame(t(cbind(min=rep(c(6,14)*7, each=3), max=rep(52*(1:3)*7, 2)))))
names(timewindows) <- sapply(timewindows, function(lmt) paste0(paste(lmt/7, collapse="-"), "w"))
...
```{r CD4absclusters, out.height=sprintf('%i%%', round(100/length(timewindows))), fig.show='hold', fig.cap=sprintf('%s clusters. Complete linkage hierarchical clustering based on Euclidean distance. Only patients with %s data during the studied time period are included in the clustering.', varname, varname)}
