...
deplsampleinfo0 <- read_csv(
  ...
  mutate(visit_nr = as.integer(str_remove(visit_nr, "^T"))) %>%
  select(sampleid, subjid, visit_nr, originalRUN) %>% 
  distinct() %>% 
...
Proteins <- proteins %>% mutate(Excluded=!protein_type_id %in% colnames(spct_proc)) %>% left_join(Proteins) %>% mutate(exp=substr(tolower(prep_type),1, 4), proteingroup=c("normalized", "excluded")[1 + 1*Excluded]) 
spct_proc <- spct_proc %>% 
    as_tibble(rownames = "sampleid") %>%
    pivot_longer(all_of(colnames(spct_proc)), "protein_type_id", values_to = "logIntensity")
```
...
spct_proc <- spct_proc %>% mutate(protein_id=gsub("-.*", "", protein_type_id), prep_type=gsub(".*-", "", protein_type_id), exp=substr(tolower(prep_type), 1, 4)) %>% left_join(Proteins %>% select(protein_type_id, Excluded, proteingroup))
...
table(Proteins$exp) %>% kable(caption="Number of proteins from each of the experiments.") %>% kable_styling(full_width = FALSE)
